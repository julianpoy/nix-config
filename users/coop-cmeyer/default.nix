{stateVersion}: {
  pkgs,
  config,
  lib,
  ...
}: {
  imports = [
    ./home-manager
  ];

  users.users.coop-cmeyer = {
    isNormalUser = true;
    description = "CoopDevCM";
    extraGroups = [
      "networkmanager"
    ];
    hashedPassword = "$y$j9T$ELyit6oRGbOFCCzGaBNie1$jW2VMNPEtW1HMF3yaDuqhjMbDI6m7dYFUNwjY/NoduD";
  };

  users.users.coop-cmeyer.openssh.authorizedKeys.keys = [
    "ssh-rsa  julianpoyourow@julianpoyourow-99m29h"
  ];

  home-manager.users.coop-cmeyer.home.stateVersion = stateVersion;
}
